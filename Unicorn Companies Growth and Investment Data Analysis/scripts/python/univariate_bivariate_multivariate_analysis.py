...
def load_cleaned_data(filepath=None):
    """Load cleaned dataset, falling back to raw + cleaning if needed."""
    if filepath is None:
        filepath = resolve_path("data", "Unicorn_Companies_cleaned.csv")
    else:
    ...
    # Fallback: run cleaning from raw data so this script works standalone
    raw_path = resolve_path("data", "Unicorn_Companies.csv")
    if not raw_path.exists():
        print("Cleaned dataset not found and raw data missing. Please run EDA script first.")
        sys.exit(1)

    print("Cleaned dataset not found. Cleaning raw data in-memory...")
    sys.path.insert(0, str(SCRIPT_DIR))
    from eda import clean_data  # local import after path setup
    df = pd.read_csv(raw_path)
    df = clean_data(df)
    out = PROJECT_ROOT / "data" / "Unicorn_Companies_cleaned.csv"
...
def univariate_analysis(df):
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'univariate_valuation.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    # Financial Stage distribution (drop missing; most rows are NaN)
    financial_stage = df['Financial Stage'].dropna().value_counts()
    if len(financial_stage) > 0:
        axes[1, 0].pie(financial_stage.values, labels=financial_stage.index, autopct='%1.1f%%')
        axes[1, 0].set_title('Financial Stage Distribution (non-missing)')
    else:
        axes[1, 0].text(0.5, 0.5, 'No Financial Stage data', ha='center', va='center')
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'univariate_categorical.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def bivariate_analysis(df):
    ...
    if len(valid_data) > 0:
        ...
        p = np.poly1d(z)
        x_sorted = valid_data['Total_Raised_B'].sort_values()
        axes[0].plot(x_sorted, p(x_sorted), "r--", alpha=0.8)
    ...
    if len(valid_data2) > 0:
        ...
        p2 = np.poly1d(z2)
        x_sorted2 = valid_data2['Investors Count'].sort_values()
        axes[1].plot(x_sorted2, p2(x_sorted2), "r--", alpha=0.8)

    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'bivariate_valuation.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'bivariate_country_valuation.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'bivariate_industry_valuation.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if len(valid_data3) > 0:
        ...
        p = np.poly1d(z)
        x_sorted3 = valid_data3['Years_to_Unicorn'].sort_values()
        plt.plot(x_sorted3, p(x_sorted3), "r--", alpha=0.8)

    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'bivariate_years_valuation.png', dpi=300, bbox_inches='tight')
    plt.close()
...
def multivariate_analysis(df):
    ...
    if len(valid_data) > 0:
        ...
        plt.tight_layout()
        plt.savefig(PLOTS_DIR / 'multivariate_pairplot.png', dpi=300, bbox_inches='tight')
        plt.close()
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'multivariate_heatmap_country_industry.png',
    ...
    if len(valid_data_3d) > 0:
        ...
        plt.tight_layout()
        plt.savefig(PLOTS_DIR / 'multivariate_3d_scatter.png', dpi=300, bbox_inches='tight')
        plt.close()
        print("Saved: multivariate_3d_scatter.png")

    # Multivariate: Violin plot (no split — Financial Stage has >2 levels)
    top_3_countries = df['Country'].value_counts().head(3).index
    filtered_df2 = df[df['Country'].isin(top_3_countries)].copy()
    filtered_df2['Financial Stage'] = filtered_df2['Financial Stage'].fillna('Unknown')
    top_stages = filtered_df2['Financial Stage'].value_counts().head(3).index
    filtered_df2 = filtered_df2[filtered_df2['Financial Stage'].isin(top_stages)]

    plt.figure(figsize=(14, 8))
    sns.violinplot(data=filtered_df2, x='Country', y='Valuation_B', hue='Financial Stage')
    ...
    plt.tight_layout()
    plt.savefig(PLOTS_DIR / 'multivariate_violin.png', dpi=300, bbox_inches='tight')
    plt.close()
